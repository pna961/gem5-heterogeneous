...
Scratchpad::Scratchpad(const Params &p)
    : ClockedObject(p),wakeupEvent([this]{wakeup();}, name()),
      accelSidePort(name() + ".accel_side_port", this),
      addrRanges(p.addrRanges.begin(), p.addrRanges.end()),chunk(p.size),
      partType(InvalidPartType), lineSize(p.lineSize), numBanks(p.numBanks),
...
void Scratchpad::processPacket(PacketPtr pkt) {
  DPRINTF(SystolicSpad, "Received request, addr %#x.\n",
          pkt->getAddr());
  Tick now = clockEdge();
  ...
  while (!returnQueue.empty() && returnQueue.front().first <= now &&
    ...
    if (!accelSidePort.sendTimingResp(pkt)) {
      DPRINTF(SystolicSpad,
              "Sending response needs retry, addr %#x.\n",
              pkt->getAddr());
      break;
    } else {
...
bool Scratchpad::AccelSidePort::sendTimingResp(PacketPtr pkt) {
  if (isStalled()) {
    ...
    DPRINTF(SystolicSpad,
            ...
            pkt->getAddr());
    return false;
  } else if (!ResponsePort::sendTimingResp(pkt)) {
...
// gem5::Scratchpad* ScratchpadParams::create() {
//   return new gem5::Scratchpad(this);
// }